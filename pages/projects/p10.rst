...
It only states that it is not required, so use it when you now it is necessary either because some
...
We will use :code:`noexpose/Dockerfile` to build an example Python server:
...
for each project, so let's use Docker Compose to run wo containers. One for the server and one for the client.
...
and we use the the :code:`docker-compose.yml` below:
...
Before you run the tests, list the exposed ports (Now should not find 8080/tcp)
...
It is not enough though to use that port from an other machine. We tried that.
...
which is just how Docker works internally at the moment. It is not necessary accessing the port.
